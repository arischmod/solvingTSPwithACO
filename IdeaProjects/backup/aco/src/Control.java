...
public class Control{
    ...
    public void Evaporate(InitializeData data)
    {
        double p = 0.5;

        for (int i=0; i<n; i++)
            for (int j=0; j<n; j++) {
                data.setPheromone(i, j, (1-p) * data.getPheromone(i,j));
                data.setPheromone(j, i, data.getPheromone(i,j));
            }
    }

    public void DepositPheromone(int k, InitializeData data)
    {
        int i,j,l;
        double trail = 1/data.getAnts(k).tour_length();

        for (i=0; i<n; i++) {
            j = data.getAnts(k).getTourCity(i);
            l = data.getAnts(k).getTourCity(i+1);
            data.setPheromone(j,l,(data.getPheromone(j,l)+trail));
    ...
    public void ASPheromoneUpdate(InitializeData data)
    {
        Evaporate(data);
        for (int k=1; k<m; k++) {
            DepositPheromone(k, data);
        }
        data.setChoiceInformation();
    }
